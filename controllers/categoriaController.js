    ...
    try {
      ...
      if (categoriasEncontradas.length <= 0) {
        return res.status(404).json({
          success: false,
          message: "Não foi encontrado nenhuma categoria registrada",
        });
      }
    ...
    try {
      ...
      if (!listaCategorias.length) {
        return res.status(400).json({
          success: false,
          message:
            "É necessário adicionar uma categoria para realizar está ação",
        });
      }

      const validacaoCamposLista = listaCategorias.map((categoria) => {
        if (!categoria.nome || categoria == null || categoria.pontos == null) {
          return {
            ...categoria,
            error: true,
      ...
      const categoriasValidas = validacaoCamposLista
        .filter((c) => !c.error)
        .map((c) => ({
          nome: c.nome,
          desconto: c.desconto,
          pontos_necessarios: c.pontos,
        }));
      const categoriasInvalidas = validacaoCamposLista
        .filter((c) => c.error)
        .map((c) => ({
          nome: c.nome,
          desconto: c.desconto,
      ...
      if (!categoriasValidas.length) {
        return res.status(400).json({
          success: false,
          message: "Nenhuma categoria enviada é válida para inserção",
          rejeitados: categoriasInvalidas,
        });
      }

      const novasCategorias = await Categoria.bulkCreate(categoriasValidas);
      ...
      await Promise.all(
        novasCategorias.map((categoria) =>
          registrarLog({
            ...
            detalhe: `Nova categoria ${categoria.nome} foi criado`,
          }),
        ),
      );
      ...
      res.status(201).json({
        ...
        inseridos: novasCategorias.length,
        rejeitados: categoriasInvalidas.length,
        categoriasInvalidas,
      });
    } catch (err) {
      console.log(err);
