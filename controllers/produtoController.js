...
module.exports = class produtoController {
    ...
    try {
      ...
      if (listaProdutos.length == 0) {
        res
          ...
          .json({
            success: false,
            message: "Favor adicionar os produtos a lista",
          });
      }
      ...
      res
        ...
        .json({
          success: true,
          message: "Adição em massa realizada",
  ...
  static async editarProduto(req, res) {
    try {
      ...
      if (!produtoCadastrado) {
        return res
          .status(404)
          .json({ success: false, message: "Produto não foi encontrado" });
      }
  ...
  static async desativarProduto(req, res) {
    try {
      ...
      const produtoCadastrado = await Produto.findOne({
        where: { id, ativo: true },
      });
      ...
      const atualizacaoProduto = await produtoCadastrado.update({
        ativo: false,
      });
