...
module.exports = class ordemVendaController{
    ...
    static async buscaOrdemVenda(req, res){
        ...
        try {
            ...
            res.status(200).json({success: true, message: "Ordens de venda encontrados", data:ordens});
    ...
    static async finalizaOrdemVenda(req, res){
        try {
            ...
            const dadosOrdemVenda = await OrdemVenda.findOne({where:{id, status:"pendente"}});

            if (!dadosOrdemVenda) {
                return res.status(404).json({success:false, message:"Ordem de venda não encontrada ou já entregue"});
            }

            const dadosPedido = await Pedido.findOne({where:{id: dadosOrdemVenda.pedido_id}});

            if(!dadosPedido){
                return res.status(404).json({success: false, message: "Pedido relacionado a Ordem de venda não encontrado"});
            }
